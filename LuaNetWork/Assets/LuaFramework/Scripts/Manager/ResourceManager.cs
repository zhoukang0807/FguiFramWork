...
namespace LuaFramework {
    ...
    public class ResourceManager : Manager {
        ...
        //异步加载一个AssetBundle
        public void LoadAssetBundle(string abName,LuaFunction func)
        {
            LoadAssetBundle<AssetBundle>(abName,null,func);
        }

        public void LoadAssetBundle(string abName,Action<UObject[]> action)
        {
            LoadAssetBundle<AssetBundle>(abName,action);
        }
        ...
        void LoadAssetBundle<T>(string abName, Action<UObject[]> action = null, LuaFunction func = null) where T : UObject
        {
            ...
            if (!m_LoadRequests.TryGetValue(abName, out requests))
            {
                ...
                requests.Add(request);
                m_LoadRequests.Add(abName, requests);
                StartCoroutine(OnLoadAssetBundle<T>(abName));
            }
            else
            {
                requests.Add(request);14
            }
        }
        ...
        IEnumerator OnLoadAssetBundle<T>(string abName) where T : UObject
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                //    //result.Add(assetObj);
                //}
                result.Add(bundleInfo.m_AssetBundle);
                if (list[i].sharpFunc != null)
                {
                    list[i].sharpFunc(result.ToArray());
                    list[i].sharpFunc = null;
